...
// obtenemos los datos del archivo
	...
	$horafoto = date("hmi");
	$horafotorand = hash('md5',$horafoto);
	$fotorand = rand(1, 60);
	$extension = explode(".",$archivo);
	$nuevoarchivo = "$horafotorand'.'$extension[1]";
	$nombrefotonuevo = "prueba$horafotorand-$fotorand.$extension[1]";
	if ($archivo != "") {
...
mysql_query("insert into baneos(nombre,razon,admin,fechaban,prueba,userid) values 
   ('$_REQUEST[nombre]','$_REQUEST[Tipoban]','$_SESSION[usuarioban]','$_REQUEST[Fechaban]','$nombrefotonuevo','$_REQUEST[userid]')", 
   $conexion) or die("Problemas en el select".mysql_error());
mysql_close($conexion);
$ar=fopen("baneos-group/archivos/baneos.txt","a") or
  ...
  if (($_REQUEST['Tipoban'] == 'Xenofobia') || ($_REQUEST['Tipoban'] == 'Bullying') || ($_REQUEST['Tipoban'] == 'Gore') || ($_REQUEST['Tipoban'] == 'Denunciar grupos al azar') || ($_REQUEST['Tipoban'] == 'Spam + cuenta FAKE-BOT') || ($_REQUEST['Tipoban'] == 'Insultos a la Administración') || ($_REQUEST['Tipoban'] == 'Venta no autorizada + cuenta FAKE-BOT') || ($_REQUEST['Tipoban'] == 'Cuenta FAKE-BOT'))
	{
	fputs($ar,"PERMANENTE");
	}
